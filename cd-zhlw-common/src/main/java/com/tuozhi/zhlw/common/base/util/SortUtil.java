...
//    }
     ...
     * 更新前台传来的json数据进行动态排序
     */
    public static String sortDynamics(String json, String beforeStr) {
        //解析json
        List<Map<String, Object>> jsonList = JsonUtil.convertJsonToList(json);
        StringBuffer sb = new StringBuffer();
        String str = "";
        if (jsonList.size() > 0) {
            //拼接字符串
            for (Map<String, Object> map : jsonList) {
                sb.append("\t" + beforeStr + map.get("property").toString() + "\t" + map.get("direction").toString() + ",");
            }
            str = sb.toString();
        }
        //截取字符串
